...
def _validate_server_name(name: str):
    """Rechaza nombres con rutas, separadores o caracteres peligrosos."""
    if not _SAFE_NAME_RE.match(name) or ".." in name or name.strip() != name:
        raise ValueError(
            "Nombre de servidor no válido. Usa solo letras, números, "
            "espacios, puntos, guiones y guiones bajos (máx. 64 caracteres)."
        )


def _validate_filename(fname: str):
    """Asegura que fname es un nombre de archivo simple, sin rutas."""
    if (not fname or fname != Path(fname).name
            or ".." in fname or "/" in fname or "\\" in fname):
        raise ValueError(f"Nombre de archivo no válido: {fname!r}")


def _safe_extract(zf: zipfile.ZipFile, dest: Path):
    """Extrae un zip validando que ninguna entrada escape del destino (zip slip)."""
    dest = dest.resolve()
    for member in zf.infolist():
        target = (dest / member.filename).resolve()
        if not target.is_relative_to(dest):
            raise ValueError(
                f"Archivo zip malicioso: la entrada {member.filename!r} "
                "intenta escribir fuera de la carpeta de destino."
            )
    zf.extractall(dest)
...
class ServerManager:
    ...
    def _get_jar_url(self, version_url):
        ...
        resp.raise_for_status()
        server = resp.json()["downloads"]["server"]
        return server["url"], server.get("sha1")

    @staticmethod
    def _verify_sha1(path: Path, expected_sha1: str):
        """Comprueba la integridad del archivo descargado contra el hash oficial."""
        import hashlib
        h = hashlib.sha1()
        ...
        if h.hexdigest() != expected_sha1:
            ...
            raise ValueError(
                "La descarga está corrupta o ha sido manipulada "
                "(el hash SHA-1 no coincide). Inténtalo de nuevo."
            )
    ...
    def create_server(self, name, version_id, server_type="vanilla", version_url=None,
        ...
        if server_type == "vanilla":
            ...
            jar_url, jar_sha1 = self._get_jar_url(version_url)
            if progress_cb:
                progress_cb("Descargando server.jar...")
            self._download_file(jar_url, server_path / "server.jar", progress_cb, "Descargando")
            if jar_sha1:
                if progress_cb:
                    progress_cb("Verificando integridad de la descarga...")
                self._verify_sha1(server_path / "server.jar", jar_sha1)
    ...
    def _server_path(self, name: str) -> Path:
        """Ruta del servidor validada: siempre dentro de SERVERS_DIR."""
        path = Path(self.servers[name]["path"]).resolve()
        if not path.is_relative_to(SERVERS_DIR.resolve()):
            raise ValueError(
                f"Ruta de servidor sospechosa fuera de {SERVERS_DIR}: {path}"
            )
        return path
    ...
    def write_properties(self, name: str, updates: dict):
        # Sin saltos de línea: evita inyectar propiedades extra vía un valor
        updates = {
            str(k).replace("\n", "").replace("\r", ""):
            str(v).replace("\n", " ").replace("\r", " ")
    ...
    def get_players(self, name: str) -> list:
        ...
        for uid, p in players.items():
            ...
            if sf.exists():
                try:
                    custom = (json.loads(sf.read_text(encoding="utf-8"))
                              .get("stats", {})
                              .get("minecraft:custom", {}))
                    ticks = (custom.get("minecraft:play_time")
                             or custom.get("minecraft:play_one_minute", 0))
                    hours = ticks / 72000
                except Exception:
                    pass
            result.append({
    ...
    def whitelist_add(self, server_name: str, uuid: str, player_name: str):
        ...
        if self.is_running(server_name):
            self.send_command(server_name, "whitelist reload")
    ...
    def op_add(self, server_name: str, uuid: str, player_name: str):
        ...
        if self.is_running(server_name):
            self.send_command(server_name, f"op {player_name}")
    ...
    def op_remove(self, server_name: str, uuid: str, player_name: str):
        ...
        if self.is_running(server_name):
            self.send_command(server_name, f"deop {player_name}")
    ...
    def ban_player(self, server_name: str, uuid: str, player_name: str):
        ...
        if self.is_running(server_name):
            self.send_command(server_name, f"ban {player_name}")
    ...
    def unban_player(self, server_name: str, uuid: str, player_name: str):
        ...
        if self.is_running(server_name):
            self.send_command(server_name, f"pardon {player_name}")
    ...
    def install_mod(self, name, jar_path):
        ...
        if src.suffix.lower() != ".jar":
            raise ValueError("Solo se pueden instalar archivos .jar")
        if not zipfile.is_zipfile(src):
            raise ValueError("El archivo no es un .jar válido")
        mods_dir = Path(self.servers[name]["path"]) / "mods"
    ...
    def restore_backup(self, name: str, backup_name: str):
        ...
        _validate_filename(backup_name)
        server_path = Path(self.servers[name]["path"])
